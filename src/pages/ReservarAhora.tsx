...
const ReservarAhora = () => {
  ...
  const nextStep = () => {
    if (currentStep === 1 && !formData.destination) {
      toast({
        title: "Error",
        description: "Por favor, selecciona un destino antes de continuar.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const templateParams = {
        ...
        martial_arts_experience: formData.martialArtsExperience,
  ...
  const renderDestinationInfo = () => {
    const destination = destinationsInfo[formData.destination];
    if (!destination) return null;

    return (
      <div className="space-y-8">
        <div>
          <h3 className="text-xl font-serif text-travel-dark mb-4">{t('booking.form.destinationInfo.highlights')}</h3>
          <ul className="list-disc list-inside space-y-2">
            {destination.highlights.map((highlight, index) => (
              <li key={index} className="text-travel-sage">{highlight}</li>
            ))}
          </ul>
        </div>

        <div>
          <h3 className="text-xl font-serif text-travel-dark mb-4">{t('booking.form.destinationInfo.requirements')}</h3>
          <ul className="list-disc list-inside space-y-2">
            {destination.requirements.map((req, index) => (
              <li key={index} className="text-travel-sage">{req}</li>
            ))}
          </ul>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div>
            <h3 className="text-xl font-serif text-travel-dark mb-4">{t('booking.form.destinationInfo.included')}</h3>
            <ul className="list-disc list-inside space-y-2">
              {destination.included.map((item, index) => (
                <li key={index} className="text-travel-sage">{item}</li>
              ))}
            </ul>
          </div>

          <div>
            <h3 className="text-xl font-serif text-travel-dark mb-4">{t('booking.form.destinationInfo.notIncluded')}</h3>
            <ul className="list-disc list-inside space-y-2">
        ...
        <div>
          <h3 className="text-xl font-serif text-travel-dark mb-4">{t('booking.form.destinationInfo.terms')}</h3>
          <ul className="list-disc list-inside space-y-2">
            {destination.terms.map((term, index) => (
              <li key={index} className="text-travel-sage">{term}</li>
            ))}
          </ul>
        </div>
        ...
        <div className="mt-8">
          <div className="flex items-center mb-6">
            ...
            <Label htmlFor="termsAccepted" className="text-sm">
              {t('booking.form.destinationInfo.acceptTerms')}
            </Label>
          </div>
          ...
          <div className="flex justify-between">
            ...
            >
              {t('booking.form.destinationInfo.continue')} <ArrowRight className="ml-2" size={18} />
            </Button>
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <section className="py-16">
        <div className="container max-w-5xl">
          ...
          <div className="mb-12">
            <div className="flex items-center justify-center">
              ...
              <div className={`flex items-center ${currentStep > 2 ? 'text-travel-terracotta' : 'text-travel-dark'}`}>
                ...
                <span className="ml-2 font-medium">{t('booking.steps.destinationInfo')}</span>
              </div>
              ...
              <div className={`w-16 h-1 mx-4 ${currentStep > 3 ? 'bg-travel-terracotta' : 'bg-gray-300'}`}></div>
              
              <div className={`flex items-center ${currentStep === 4 ? 'text-travel-terracotta' : 'text-travel-dark'}`}>
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                  currentStep === 4 ? 'bg-travel-terracotta text-white' : 'bg-travel-cream text-travel-dark'
                }`}>
                  4
                </div>
          ...
          {currentStep === 2 && (
            <div className="bg-white p-8 rounded-lg shadow-sm border border-gray-100">
              <h2 className="text-3xl font-serif text-travel-dark mb-6">{t('booking.form.destinationInfo.title')}</h2>
              <p className="text-travel-sage mb-8">{t('booking.form.destinationInfo.subTitle')}</p>
